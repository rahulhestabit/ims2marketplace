...
It's part of [marketplace project - first Progressive Web App for eCommerce](https://github.com/DivanteLtd/marketplace) with Magento2 support.
...
Categories and Product-to-categories links are additionaly stored in Redis cache for rapid-requests (for example from your WebAPI). Our other project [marketplace-api](https://github.com/DivanteLtd/marketplace-api) exposes this database to be used in PWA/JS webapps.
...
**Tutorial on installation / integration [manual for Vue Storefront connectivity](https://medium.com/@piotrkarwatka/marketplace-how-to-install-and-integrate-with-magento2-227767dd65b2)**
...
[marketplace (Github)](https://github.com/DivanteLtd/marketplace) — is the main project where you can also find most of the documentation, issues mapped to further releases and other resources to start with — Vue.js on webpack.

[marketplace-api (Github)](https://github.com/DivanteLtd/marketplace-api) — is the API layer which provides the data to marketplace app — Node.js, Express; This project consist of docker instances for Redis and ElasticSearch required by ims2marketplace and pimcore2vuestorefront

ims2marketplace — THIS project -data bridges which are in charge of moving data back from Magento2 to Vue Storefront data store.

You must install `marketplace-api` locally. You may install it using the Vue Storefront installer - see details. Or manually by executing the sequence of commands:

```bash
git clone https://github.com/DivanteLtd/marketplace-api
cd marketplace-api
npm install
npm run migrate
docker-compose up -d
...
The key command is `docker-compose up -d` which runs the ElasticSearch and Redis instances - both required by `ims2marketplace`

### Elastic 7 Support

By default, Vue Storefront API docker files and config are based on Elastic 5.6. We plan to change the default Elastic version to 7 with the 1.11 stable release. As for now, the [Elastic 7 support](https://github.com/DivanteLtd/marketplace-api/pull/342) is marked as **experimental**. 
...
Now, You're ready to run the importer. Please check the [config file](https://github.com/DivanteLtd/ims2marketplace/blob/master/src/config.js). You may setup the Magento access data and URLs by config values or ENV variables.
...
Please note:
After data import - especially when You're not sure about the product attributes data types - please **reindex** ElasticSearch to establish the correct / current database schema. You may do this using [Database tool](https://github.com/DivanteLtd/marketplace/blob/master/doc/Database%20tool.md) in the `marketplace-api` folder:
...
If you want to see how many products were stored into Elastic data store, you can use Kibana to do so. Kibana is part of marketplace-api. Once you start docker containers of marketplace-api you can access it on http://localhost:5601/.
...
In the current version the webhook notifies ims2marketplace about changed product SKUs and ims2marketplace pulls the modified products data via Magento2 APIs.

First. You should install [Magento2 module called VsBridge](https://github.com/DivanteLtd/magento2-vsbridge)
Second. Deploy ims2marketplace on the server.
...
Anyway - this API must be publicly available via Internet OR You must have the ims2marketplace installed on the same machine like Magento2.
...
Webapi will [add the products to the queue](https://github.com/DivanteLtd/ims2marketplace/blob/b44e7ede9aeb27f308e2a87033251a2491640da8/src/api/routes/magento.js#L19).
...
Multiwebsite support starts with the ElasticSearch indexing. Basically - each store has it's own ElasticSearch index and should be populated separately using [ims2marketplace](https://github.com/DivanteLtd/ims2marketplace) tool.
...
Then, to use these indexes in the Vue Storefront You should index the database schema using the `marketplace-api` db tool:
...
Please make sure that You've got the `config.synchronizeCatalogSpecialPrices` (env: `PRODUCTS_SPECIAL_PRICES`) and `config.renderCatalogRegularPrices` (env: `PRODUCTS_RENDER_PRICES`) set to `true` (default is `false`). Otherwise only the catalog prices will be synced (without dynamic pricing rules applied). You can also use the Vue Storefront [dynamic-pricing option](https://divanteltd.github.io/marketplace/guide/integrations/direct-prices-sync.html) for the same purpose.
